...
Keep these in sync with the design tokens in [`UI.md`](../UI.md) §2 — primary
`#7c3aed` / `#8b5cf6`, Inter for type, light base `#f8fafc` / dark base `#0b1020`.
...
> a transparent background, optionally with a subtle **flat two-stop** violet
> gradient (no mesh, no 3D). Flat, modern, premium, institutional — like a cross
> between a bank seal and a privacy app. Centered, symmetrical, generous padding.
> No text, no letters, no mascot, no 3D, no drop shadows, no photorealism. Vector,
> SVG-style, crisp edges, scalable, works as a small favicon.

_(Add tool-specific flags from §6 — e.g. Midjourney `--style raw --v 6 --ar 1:1`.)_
...
- No 3D bevels, glossy reflections, drop shadows, gradient *meshes*, or photoreal
  gradients. (A subtle flat two-stop violet gradient, per §1, is fine.)
...
2. **On-dark** — mark in white or light violet `#A78BFA` for the dark theme (base `#0b1020`).
3. **On-light** — mark in `#6D28D9` for the light theme (base `#f8fafc` / white).
...
- [ ] Works on both light (`#f8fafc`) and dark (`#0b1020`) backgrounds (the shipped theme bases).
