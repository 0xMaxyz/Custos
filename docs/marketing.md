# Custos — Community & Marketing Assets
...
mainnet deploy + public frontend are live.
...
> captions on any video clips. Use the community-cut clip ([demo.md](./demo.md) §8) on tweet 1.
...
- Use the Demo-states panel to seed a de-risk state cleanly without needing a live mainnet event for screenshots.
- For screenshot 7, trigger a real `deRisk` on testnet first and screenshot that tx (testnet mantlescan also counts for the screenshot).
...
> ([demo.md](./demo.md)), just cut tight. Export at ~15 MB so it uploads natively to X.
...
to whatever the Demo-states panel actually seeds when you capture.
