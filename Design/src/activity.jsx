...
(function () {
  ...
  // ---------- Timeline item ----------
  function DecisionItem({ d, onOpen }) {
    const r = S.RISK[d.riskLevel];
    return (
      <button className="decision-item" onClick={() => onOpen(d)} aria-label={`Decision ${d.id} detail`}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 9, flexWrap: "wrap" }}>
            ...
            <span style={{ fontSize: "0.8125rem", color: "var(--faint)", marginLeft: "auto" }}>{f.timeAgo(d.timestamp)}</span>
          </div>
  ...
  function ActivityPage({ openDecision, loading, activityError: errorProp }) {
    const [filter, setFilter] = useState("all");
    const [risk, setRisk] = useState("all");
    const [error, setError] = useState(false);
    const showError = error || !!errorProp;
    ...
    return (
      <div className="page">
        ...
        {loading ? (
          <div className="grid" style={{ gap: 12 }}>{[0, 1, 2].map((i) => <window.Skeleton key={i} h={190} r={14} />)}</div>
        ) : showError ? (
          <Card><window.ErrorState title="Couldn't load decisions" body="The agent history couldn't be fetched. Check your connection and try again." onRetry={() => setError(false)} /></Card>
        ) : list.length === 0 ? (
          ...
          <div className="grid" style={{ gap: 12 }}>
            {list.map((d) => <DecisionItem key={d.id} d={d} onOpen={openDecision} />)}
          </div>
        )}
      </div>
    );
  ...
  function DecisionDetailModal({ decision: d, onClose }) {
    const r = S.RISK[d.riskLevel];
    const evById = Object.fromEntries(d.evidence.map((e) => [e.id, e]));
    return (
      <window.Modal title={`Decision #${d.id}`} icon={KIND[d.kind].icon} onClose={onClose} size="lg">
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center", marginBottom: 16 }}>
          <KindBadge kind={d.kind} />
          <RiskLevelChip level={d.riskLevel} />
          <ConfidenceMeter value={d.confidence} />
          <span style={{ flex: 1 }} />
          <span style={{ fontSize: "0.8125rem", color: "var(--muted)" }}>{f.dateTime(d.timestamp)}</span>
        </div>
        ...
        <Section title="Allocation — before → after">
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <GuardrailsMark small />
            <span style={{ fontSize: "0.8125rem", color: "var(--muted)" }}>Ceiling in force: max USDY <span className="mono">{f.bpsToWeight(d.maxUsdyWeightBpsAllowed)}%</span></span>
          </div>
        </Section>
        ...
        <Section title="Outcome">
          <div style={{ padding: "14px 16px", borderRadius: "var(--rounded-btn)", background: "var(--base-200)" }}>
            ...
            <div style={{ fontSize: "0.75rem", color: "var(--faint)", marginTop: 10 }}>Measured {f.dateTime(d.outcome.measuredAt)}</div>
          </div>
        </Section>
        ...
        <Section title="Verifiability">
          ...
          <div className="kvrow"><span className="k">Rationale hash</span><span className="mono v" style={{ fontSize: "0.8125rem" }}>{f.shortHash(d.rationaleHash, 10, 6)}</span></div>
          {d.evidenceHash && <div className="kvrow"><span className="k">Evidence hash</span><span className="mono v" style={{ fontSize: "0.8125rem" }}>{f.shortHash(d.evidenceHash, 6, 4)}</span></div>}
          <div className="kvrow"><span className="k">Decision bundle</span><a className="linklike mono" style={{ fontSize: "0.8125rem" }} href="#" onClick={(e) => e.preventDefault()}>{f.shortHash(d.decisionURI, 14, 6)} <Icon name="external-link" size={13} /></a></div>
        </Section>
      </window.Modal>
    );
  }

  Object.assign(window, { ActivityPage, DecisionDetailModal });
})();
