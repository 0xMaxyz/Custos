...
contract ForkTest is Test {
    ...
    function testErc8004PresenceOnMantle() public view {
        ...
        if (idSize == 0) {
            console2.log("[0.3] WARN: ERC-8004 singletons absent on Mantle - must deploy own registries");
        } else {
    ...
    function testLiquidityGateUsdy() public {
        ...
        for (uint256 i = 0; i < amounts.length; i++) {
            ...
            uint256 slippageBps = expected18 > usdyOut
                ? ((expected18 - usdyOut) * 10_000) / expected18
                : 0;
    ...
    function testUsdyTransferHookNotBlocked() public {
        ...
        assertEq(IERC20Minimal(USDY).balanceOf(recipient), usdyHeld / 2, "recipient balance mismatch");
        ...
        if (hasBlocklist && blData.length == 32) {
            ...
            assertFalse(blocked, "test contract IS blocklisted - must use a different vault address");
            console2.log("[0.5] USDY.isBlocked(address(this)):", blocked);
        } else {
            console2.log("[0.5] USDY blocklist interface not directly accessible - transfer test is sufficient");
        }
    }
    ...
    {
        uint256[] memory steps = new uint256[](1); steps[0] = binStep;
        uint8[]   memory vers  = new uint8[](1);   vers[0]  = version;
        address[] memory toks  = new address[](2);  toks[0] = t0; toks[1] = t1;
        return abi.encode(steps, vers, toks);
    }

    function _buildPath2Hop(
        address t0, address t1, address t2,
        uint256 bs0, uint256 bs1, uint8 v0, uint8 v1
    ) internal pure returns (bytes memory) {
        uint256[] memory steps = new uint256[](2); steps[0] = bs0; steps[1] = bs1;
        uint8[]   memory vers  = new uint8[](2);   vers[0]  = v0;  vers[1]  = v1;
        address[] memory toks  = new address[](3);  toks[0] = t0; toks[1] = t1; toks[2] = t2;
        return abi.encode(steps, vers, toks);
    }
}
