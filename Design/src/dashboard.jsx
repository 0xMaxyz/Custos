...
(function () {
  ...
  // ---------- Agent status card (trust anchor) ----------
  function AgentStatusCard({ decision, go }) {
    const r = S.RISK[decision.riskLevel];
    return (
        ...
        <div style={{ display: "flex", gap: 18, alignItems: "flex-start", flexWrap: "wrap" }}>
          <div style={{ flex: "1 1 280px" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 14, marginTop: 16, flexWrap: "wrap" }}>
              <span style={{ fontSize: "0.8125rem", color: "var(--faint)" }}>Last action {f.timeAgo(decision.timestamp)} · #{decision.id}</span>
          ...
          <div style={{ flex: "1 1 240px", borderLeft: "1px solid var(--border)", paddingLeft: 18, minWidth: 220 }} className="agent-status-side">
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
              {decision.signals.slice(0, 2).map((s, i) => <window.SignalBadge key={i} type={s.type} severity={s.severity} />)}
              <window.ConfidenceMeter value={decision.confidence} compact />
            </div>
          </div>
        </div>
      </Card>
  ...
  // ---------- Baseline counter (the Turing-test answer) ----------
  function BaselineCounter({ b }) {
    return (
        ...
        <div style={{ display: "flex", gap: 24, alignItems: "center", flexWrap: "wrap" }}>
          <div style={{ flex: "1 1 auto" }}>
            <div style={{ display: "flex", alignItems: "baseline", gap: 8, flexWrap: "wrap" }}>
              <span className="mono" style={{ fontSize: "2.25rem", fontWeight: 700, letterSpacing: "-0.02em", color: "var(--success)", lineHeight: 1 }}>{f.pctSigned(b.passiveDeltaBps)}</span>
              <span style={{ color: "var(--muted)", fontSize: "0.9375rem" }}>vs passive</span>
            </div>
            <div style={{ display: "flex", gap: 22, marginTop: 14, flexWrap: "wrap" }}>
              <div>
                <div className="mono" style={{ fontWeight: 600, fontSize: "1.0625rem", color: "var(--success)" }}>−{f.usd(b.drawdownAvoidedUsdc)}</div>
                <div style={{ fontSize: "0.75rem", color: "var(--muted)" }}>drawdown avoided</div>
              </div>
              <div>
                <div className="mono" style={{ fontWeight: 600, fontSize: "1.0625rem" }}>{f.pctSigned(b.realizedYieldBps)}</div>
  ...
  // ---------- Position card ----------
  function PositionCard({ connected, empty, position, vault, paused, killed, onDeposit, onWithdraw, onConnect }) {
    if (!connected) {
      return (
        <Card>
    ...
    return (
      <Card>
        <div className="card-hl">
          ...
          <span className="chip role-success" style={{ height: 22 }}>{f.bpsToPct(vault.blendedApyBps)} blended APY</span>
        </div>
        <div style={{ display: "flex", alignItems: "flex-end", gap: 16, flexWrap: "wrap" }}>
          <div>
            <div className="stat-label">Current value</div>
            <div className="mono" style={{ fontSize: "2rem", fontWeight: 700, letterSpacing: "-0.02em", lineHeight: 1.1 }}>{f.usd(position.valueUsdc)}</div>
          </div>
          <div style={{ marginBottom: 4 }}>
            <span className="chip role-success"><Icon name="trending-up" size={13} />{f.usd(yld, { sign: true })} all-time</span>
          </div>
        </div>
        <div style={{ marginTop: 16 }}>
          <div className="kvrow"><span className="k">Deposited</span><span className="v mono">{f.usd(position.depositedUsdc)}</span></div>
          <div className="kvrow"><span className="k">Shares</span><span className="v mono">{f.num(position.shares)}</span></div>
          <div className="kvrow"><span className="k">Share price</span><span className="v mono">{f.price(position.sharePrice)}</span></div>
        </div>
    ...
    return (
      <Card>
        ...
        <div style={{ display: "flex", gap: 22, alignItems: "center", flexWrap: "wrap" }}>
          <div style={{ position: "relative", flexShrink: 0 }}>
            ...
            <div style={{ position: "absolute", inset: 0, display: "grid", placeItems: "center", textAlign: "center" }}>
              <div>
                <div className="mono" style={{ fontWeight: 700, fontSize: "1.0625rem" }}>{f.usd(tvl, { cents: false })}</div>
        ...
        <div style={{ marginTop: 16, padding: "12px 14px", borderRadius: "var(--rounded-btn)", background: ok ? "var(--success-soft)" : "var(--error-soft)", display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <span style={{ fontSize: "0.875rem", flex: 1 }}>Instantly withdrawable <strong className="mono">{f.usd(instant)}</strong> ({instantPct}% of TVL)</span>
      ...
      <Card>
        ...
        <div className="grid" style={{ gridTemplateColumns: "1fr 1fr", gap: 18 }}>
          <div>
            ...
            <div className="mono" style={{ fontWeight: 600, fontSize: "1.125rem", marginTop: 4 }}>{f.usd(tvl, { cents: false })} <span style={{ color: "var(--faint)", fontWeight: 500 }}>/ {f.usd(cap, { cents: false })}</span></div>
            ...
            <div style={{ fontSize: "0.75rem", color: "var(--muted)", marginTop: 4 }}>{usedPct}% used · {f.usd(cap - tvl, { cents: false })} remaining</div>
          </div>
          <div>
            ...
            <div className="mono" style={{ fontWeight: 600, fontSize: "1.125rem", marginTop: 4, color: "var(--success)" }}>{f.bpsToPct(vault.blendedApyBps)}</div>
            {open && (
              <div style={{ marginTop: 8, fontSize: "0.8125rem" }}>
                <div className="kvrow" style={{ padding: "4px 0" }}><span className="k">USDY implied APY</span><span className="v mono" style={{ fontSize: "0.8125rem" }}>{f.bpsToPct(vault.usdyImpliedApyBps)}</span></div>
                <div className="kvrow" style={{ padding: "4px 0" }}><span className="k">Aave supply APY</span><span className="v mono" style={{ fontSize: "0.8125rem" }}>{f.bpsToPct(vault.aaveUsdcSupplyApyBps)}</span></div>
        ...
        <div className="grid" style={{ gridTemplateColumns: "1fr 1fr", gap: 18 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: "0.8125rem", color: "var(--muted)", marginTop: 8 }}>Range valid until <span className="mono">{f.dateShort(vault.oracleRangeEnd)}</span></div>
  ...
  function DashboardPage({ connected, paused, killed, emptyPosition, go, onDeposit, onWithdraw, onConnect, loading }) {
    ...
    return (
      <div className="page">
        ...
        <div className="grid" style={{ gap: 16 }}>
          <AgentStatusCard decision={decision} go={go} />
          <BaselineCounter b={S.baseline} />
          <div className="grid dash-cols">
            <PositionCard connected={connected} empty={emptyPosition} position={S.position} vault={S.vault} paused={paused} killed={killed} onDeposit={onDeposit} onWithdraw={onWithdraw} onConnect={onConnect} />
            <AllocationCard vault={S.vault} />
          </div>
          <VaultStatsCard vault={S.vault} />
        </div>
      </div>
    );
  }
