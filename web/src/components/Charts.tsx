// Data-viz (§7). Matches Design/src/charts.jsx.

import { useState } from "react";
import { Icon } from "./Icons";
import * as fmt from "../lib/fmt";
import { BUCKETS, BUCKET_LABEL, type WeightsBps, type BucketKey } from "../lib/data";
...
export function AllocationChart({ weightsBps, size = 168, stroke = 22 }: { weightsBps: WeightsBps; size?: number; stroke?: number }) {
...
export function AllocationLegend({ weightsBps, tvlUsdc }: { weightsBps: WeightsBps; tvlUsdc: string }) {
      ...
      {BUCKETS.map((b) => {
        ...
        return (
          <div key={b} style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <span className="mono" style={{ fontSize: "0.875rem", fontWeight: 600 }}>{fmt.bpsToWeight(w)}%</span>
            <span className="mono" style={{ fontSize: "0.8125rem", color: "var(--muted)", minWidth: 70, textAlign: "right" }}>{fmt.usd(usd)}</span>
          </div>
        );
      })}
    </div>
  ...
  const Bar = ({ w, label }: { w: WeightsBps; label: string }) => (
    <div style={{ flex: 1 }}>
      ...
      <div style={{ display: "flex", height: 12, borderRadius: 4, overflow: "hidden", background: "var(--base-300)" }} role="img"
        aria-label={`${label} allocation ${BUCKETS.map((b) => `${b} ${fmt.bpsToWeight(w[b])}%`).join(", ")}`}>
        {BUCKETS.map((b) => w[b] > 0 ? (
          <span key={b} style={{ width: (w[b] / 100) + "%", background: bucketColor[b], transition: "width 0.4s var(--ease)" }} title={`${BUCKET_LABEL[b]} ${fmt.bpsToWeight(w[b])}%`} />
        ) : null)}
      </div>
    </div>
  );
...
export function PegGauge({ deviationBps, warn = 30, block = 50, derisk = 100 }: { deviationBps: number; warn?: number; block?: number; derisk?: number }) {
  const maxScale = 130;
  const pct = Math.min(deviationBps / maxScale, 1) * 100;
  const mark = (v: number) => Math.min(v / maxScale, 1) * 100;
  ...
  return (
    <div>
      ...
      <div style={{ position: "relative", height: 8, borderRadius: 99, background: "linear-gradient(90deg, var(--success-soft), var(--warning-soft) 45%, var(--error-soft))" }}>
        ...
        {([["warn", warn], ["block", block], ["derisk", derisk]] as [string, number][]).map(([lab, v]) => (
...
// ---------- LiquidityBufferBar ----------
export function LiquidityBufferBar({ pct, floor = 15 }: { pct: number; floor?: number }) {
  const ok = pct >= floor;
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 7 }}>
...
interface ChartSeries { key: string; label: string; color: string; data: { t: string; v: number }[]; }

export function LineChart({ series, height = 180, yFmt = (v: number) => String(v), yPad = 0.0008, markers = [] }: {
  series: ChartSeries[]; height?: number; yFmt?: (v: number) => string; yPad?: number; markers?: { i: number; label: string }[];
}) {
  ...
  const path = (data: { t: string; v: number }[]) => data.map((d, i) => `${i ? "L" : "M"}${x(i).toFixed(1)} ${y(d.v).toFixed(1)}`).join(" ");
      ...
      {!showTable ? (
        ...
        <div style={{ overflowX: "auto" }}>
          <table className="dtable">
            ...
            <tbody>
              {labels.map((lab, i) => (
                <tr key={lab}><td className="mono">{lab}</td>{series.map((s) => <td key={s.key} className="mono">{yFmt(s.data[i]?.v ?? 0)}</td>)}</tr>
              ))}
            </tbody>
          </table>
        </div>
