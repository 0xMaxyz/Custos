  ...
  return (
    <Card>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
        ...
        <span style={{ fontSize: "0.875rem", color: "var(--muted)" }}>valid until <span className="mono">{fmt.dateShort(ins.oracleRangeEnd)}</span></span>
      </div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", marginTop: 6, fontSize: "0.75rem", color: "var(--faint)" }}>
        <span className="mono">{fmt.dateShort(ins.oracleRangeStart)}</span>
        <span>range ends in 16 days</span>
        <span className="mono">{fmt.dateShort(ins.oracleRangeEnd)}</span>
      </div>
  ...
  return (
    <Card>
      <div className="card-hl">
        ...
        <span style={{ fontSize: "0.8125rem", color: "var(--muted)" }} className="mono">{fmt.usd(ins.aaveWithdrawableUsdc, { cents: false })} withdrawable</span>
      </div>
...
export function InsightsPage({ loading }: { loading: boolean }) {
  if (loading) {
