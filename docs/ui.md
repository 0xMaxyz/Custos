...
Design spec for the frontend. Stack is fixed: **React + Vite + Tailwind + daisyUI**, wallet via
...
- **Allocation card:** donut or stacked bar across IDLE / AAVE / USDY / AUSD with %
  ...
  1:1-by-NAV via the Ondo wrap/unwrap converter); show the form split as a
...
**Purpose:** the transparent, on-chain decision log.
...
- **Agent economics (Could):** a compact panel that makes the agent a verifiable
  ...
  - **x402:** "buys its evidence, sells its judgment." Show premium feeds the
    ...
    it exposes — `GET /risk-score`, price (e.g. 0.01 USDC/call), `payTo`.
  - **Verifiable jobs (ERC-8183):** a small ledger of de-risk Jobs — status
    (`Completed`/`Rejected`), the **guardrail Evaluator** verdict, bounty, and the
    ERC-8004 reputation entry each wrote.
...
- **Pays for its evidence:** premium evidence carries a "Paid" badge with the x402
  receipt — the agent puts money behind the data it acts on, and sells its own
  risk score, making it a transparent economic actor (never from the custody path).
- **Every de-risk is a settled, verifiable job:** the de-risk's ERC-8183 Job
...
**AGENT** = off-chain agent API / LLM verdict (see [spec.md §3](./spec.md)) · **CHAIN** = wallet/RPC.
...
**Agent economics — x402 + verifiable jobs (Agent §5.3, Activity §5.2)**
| UI element | Field | Unit | Example | Source |
|---|---|---|---|---|
...
```jsonc
{
  ...
  "x402": {                             // pays for evidence, sells its risk score
    "paidEvidence": [
  ...
  "jobs": [                             // each de-risk as an ERC-8183 verifiable Job
