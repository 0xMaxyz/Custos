...
contract YieldVault is ERC4626, AccessControl, Pausable, ReentrancyGuard {
  ...
  constructor(
    ...
    address admin,
    address _guardrails
  ) ERC4626(IERC20(usdc)) ERC20("Custos Yield Vault", "cvUSDC") {
  ...
  ) external onlyRole(Roles.GUARDIAN) {
   ...
   */
  function convertRwaLeg(
    bool toMusd,
    uint256 amountIn,
    uint256 minOut
  ) external onlyRole(Roles.ALLOCATOR) whenNotPaused nonReentrant returns (uint256 amountOut) {
    _requireNotKilled();

    IUsdyAdapter rwa = IUsdyAdapter(address(adapters[BUCKET_USDY]));
    if (address(rwa) == address(0)) revert RwaAdapterNotSet();

    amountOut = toMusd ? rwa.convertToMusd(amountIn, minOut) : rwa.convertToUsdy(amountIn, minOut);
  ...
  function _buildMarketState(
    uint256 tvl,
    uint256 usdyDexSpotUsdc
  ) internal view returns (Guardrails.MarketState memory s) {
